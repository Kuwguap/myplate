...
export class DocumentController {
  static async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await db.run(
        'INSERT INTO documents (name, template_id, data) VALUES (?, ?, ?)',
        [name, template_id, JSON.stringify(validatedData)]
      )
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Update document
      await db.run(
        'UPDATE documents SET name = COALESCE(?, name), data = COALESCE(?, data), status = COALESCE(?, status) WHERE id = ?',
        [
          name,
